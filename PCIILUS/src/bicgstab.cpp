               ...
               int *info)
{
    ...
    double *Av = &(A.v[0]);
    int *Ui = &(U.i[0]), *Uj = &(U.j[0]), *Gi = &(G.i[0]), *Gj = &(G.j[0]);
    double *Uv = &(U.v[0]), *Gv = &(G.v[0]);
    int *Li, *Lj, *Hi, *Hj;
    double *Lv, *Hv;
    if (!issym)
    {
    ...
    sparse_matrix_t AA, LL, HH, UU, GG;
    struct matrix_descr descrA, descrU, descrL, descrG, descrH;
    ...
    {
        // p_c = U^{-1} D^{-1} L^{-1} p
        apply(n, nr, &GG, &descrG, Dv, &HH, &descrH, &LL, &descrL, &UU, &descrU, p, pc, tm);

        // v = A p_c
        mkl_sparse_d_mv(SPARSE_OPERATION_NON_TRANSPOSE, 1.0, AA, descrA, pc, 0.0, v);

        // alpha = m1 / (r_0,v)
        double dnm = cblas_ddot(n, r0, 1, v, 1);
        a = m1 / dnm;
        ...
        // s_c = U^{-1} D^{-1} L^{-1} s
        apply(n, nr, &GG, &descrG, Dv, &HH, &descrH, &LL, &descrL, &UU, &descrU, s, sc, tm);
        ...
        if (!std::isfinite(rc))
        {
            if (verbose)
            {
                std::cout << "BiCGSTAB NaN or Inf detected at iteration " << *iter << "\n";
                (*info) = 2;
                break;
            }
        }
